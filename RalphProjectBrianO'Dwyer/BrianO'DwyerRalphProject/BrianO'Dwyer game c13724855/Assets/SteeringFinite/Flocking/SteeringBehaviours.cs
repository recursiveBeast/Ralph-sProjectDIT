...
public class SteeringBehaviours : MonoBehaviour {
	...
	Vector3 WallAvoid(){
		RaycastHit2D detector = 
			Physics2D.Raycast (transform.position, rb.velocity.normalized * wallAvoidDistance, wallAvoidDistance, whatToAvoid);
	...
	public Vector3 Seperation(){
		...
		// iterate through the tagged game objects
	...
	public Vector3 Cohesion(){
		...
		if (taggedCount > 0) {
			...
			// if we are already at the centre of mass, we use sqrmagnitude because it's let processing
	...
	public Vector3 Alignment(){
		...
		int taggedCount = 0;
		// iterate through the tagged List
		foreach (GameObject entity in tagged) {
	...
	private bool AccumulateForce(ref Vector3 runningTotal, Vector3 force){
		...
		if (toAdd < remaining) {
			// we have enough remaining force budget left so just add the force to the total
			runningTotal += force;
		} else {
